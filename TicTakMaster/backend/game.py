"""
This module has the core game logic and AI implementation using
...
class TicTacToeAI:
    ...
    def get_best_move(self, game, use_alpha_beta=True):
        ...
        for move in available_moves:
            ...
            if use_alpha_beta:
                score = self._minimax_alpha_beta(game_copy, 0, False, float('-inf'), float('inf'))
            else:
    ...
    def _minimax_alpha_beta(self, game, depth, is_maximizing, alpha, beta):
        """
        ...
        if is_maximizing:
            ...
            for move in available_moves:
                ...
                game_copy = self._copy_game(game)
                game_copy.make_move(row, col)
                score = self._minimax_alpha_beta(game_copy, depth + 1, False, alpha, beta)
                best_score = max(score, best_score)
            ...
            for move in available_moves:
                ...
                game_copy = self._copy_game(game)
                game_copy.make_move(row, col)
                score = self._minimax_alpha_beta(game_copy, depth + 1, True, alpha, beta)
                best_score = min(score, best_score)
...
def play_console_game():
    ...
    while not game.game_over:
        ...
        if game.current_player == 'X':
            ...
            nodes_after = ai.nodes_explored
            print(f"AI explored {nodes_after - nodes_before} nodes")
            row, col = move
